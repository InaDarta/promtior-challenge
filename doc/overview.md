...
Esto no es solo una ficha técnica. Es el relato de cómo encaramos, en siete días, un challenge que
pide un chatbot con tool calling para reservar salas de reunión — qué decidimos y por qué, qué nos
salió mal en el camino y cómo lo resolvimos, y qué hicimos con cada hueco que el enunciado dejaba
...
## Cómo encaramos el problema

El enunciado deja el lenguaje y el framework libres, y ahí ya tuvimos que elegir un lado. Podíamos
irnos al ecosistema donde LangChain nació — Python — y movernos rápido con lo que ya existe, o
apostar por Java con tipos fuertes e inmutables (`record`, `sealed interface`) para que una reserva
inválida sea **irrepresentable**, no solo rechazada en tiempo de ejecución. Elegimos lo segundo,
sabiendo que perdíamos comunidad y ejemplos de LangChain4j frente a su primo Python. El
razonamiento completo, con las alternativas que descartamos (Python+FastAPI, Node/TypeScript,
Spring AI, Quarkus, H2/SQLite), quedó en [ADR 0001](adr/0001-stack.md).
...
fondo, la respuesta a una sola pregunta que nos hicimos desde el día uno: ¿cómo hacemos para que
eso sea cierto de verdad, no solo una buena intención en el prompt?
...
2. `BookingAssistant` arma el turno con un system prompt dinámico (`BookingSystemPrompt`: rol,
   ...
   automático a Groq (más abajo contamos por qué hizo falta).
...
## Metodología: cómo trabajamos estos siete días

La parte técnica es una mitad de la historia. La otra es cómo la construimos, porque también dice
algo de cómo encaramos el problema.

Trabajamos con issue-driven development desde el primer día: el plan completo vive en
...
que nos permitió no perder el hilo en siete días de trabajo intenso, y lo que hace que el
...
Y acá va la parte que quizás no se nota mirando solo el código: este desarrollo es generativo.
Cada sub-issue lo ejecuta Claude Code — el mismo tipo de agente que está escribiendo este
documento — trabajando en un worktree de git aislado por rama, siguiendo las reglas que dejamos
fijadas en `CLAUDE.md` (arquitectura hexagonal estricta, Spotless obligatorio, un ADR por decisión
con su alternativa descartada, PR siempre contra `develop`). Nosotros nos concentramos en lo que un
agente no puede resolver solo: priorizar qué sub-issue sigue, revisar cada PR antes de mergearlo, y
tomar las decisiones de producto que el enunciado deja abiertas — la escalera de capacidades de
sala es el ejemplo más claro. Este mismo documento es un sub-issue más (`E09.1`), con su alcance y
su criterio de aceptación definidos de la misma forma que cualquier otro.

Organizamos el trabajo en tres milestones que funcionaron como mini-sprints sucesivos hacia un
deadline fijo de siete días desde que arrancó el challenge (2026-08-27):

- **M1 Core** (`E00`–`E03`): el sistema de reservas correcto y probado, sin IA todavía. Queríamos
  que el dominio y la persistencia estuvieran sólidos antes de ponerle un modelo de lenguaje
  encima — construir sobre una base que todavía no confiábamos hubiera sido apilar riesgo sobre
  riesgo.
- **M2 Agente** (`E04`–`E06`): los casos de uso, la API REST, el agente conversacional con tool
  calling y su interfaz web. El corazón evaluable del challenge, y también donde más sorpresas nos
...
Desde el principio marcamos el camino crítico (`E00 → E01 → E02 → E03 → E04 → E05 → E08 → E09`) y
también qué estábamos dispuestos a sacrificar si el tiempo apretaba: `E06` (interfaz) y `E07`
...
prueba de que funcionó es que, al final, no tuvimos que cortar nada de eso: streaming, agenda y
trazas terminaron entrando igual ([ADR 0010](adr/0010-streaming-del-chat.md),
[ADR 0011](adr/0011-trazas-de-llm-y-tool-calls-con-langfuse.md)). Donde el reloj sí se sintió de
verdad fue en `E07.3`, la suite de evaluación en vivo: la corrimos con 2-3 días de plazo restantes,
y en vez de dejarla a medias sin decirlo, documentamos explícitamente que solo alcanzamos a correr
8 de las 20 frases planeadas — un recorte declarado, no una corrida que se abandonó en silencio.

## La lógica de fondo: el dominio decide, el modelo orquesta

Esta es la regla de diseño que más nos importaba sostener, y no la dejamos como una buena
intención: se sostiene con tres mecanismos concretos, no con una convención que dependa de que
nadie la rompa.
...
natural para que el modelo pueda explicarlas, pero le dijimos explícitamente que **no las aplique
por su cuenta**: "Vos no aplicás estas reglas, las aplica el sistema al llamar a la tool. No le
...
Esa instrucción no es cosmética, y lo comprobamos de la peor manera posible: en vivo. La suite de
...
pasó por el dominio, así que cuenta como desacierto bajo nuestro propio criterio estricto. Fue un
buen recordatorio de algo que ya sospechábamos: sin una frontera dura entre "explicar" y "decidir",
un modelo que razona bien igual se puede saltar la única fuente de verdad, y no hay forma de
notarlo mirando solo la respuesta final.
...
rama `default` — si mañana agregamos una regla nueva, el compilador nos obliga a manejarla en los
dos lugares que la consumen: el contrato REST `problem+json` (`BookingProblems`) y el resultado
...
tres de los ocho casos que corrimos son justamente intentos de suplantación o de ingeniería social
...
Esa garantía nos hizo pelear con un detalle técnico nada obvio: `SecurityContextHolder` es
...
invocarla desde ese hilo — nos pasó de verdad, ver la próxima sección. `SecurityContextToolExecutor`
...
límite de una conversación, y eso hay que resolverlo dos veces si tenés dos cosas thread-local.

## Los desafíos que nos encontramos (y cómo salimos de cada uno)

Esto es lo más honesto que podemos contar: qué nos salió mal, en qué orden lo descubrimos, y qué
tuvimos que hacer para salir. Ninguno de estos problemas estaba en el plan original.

**Un mismo síntoma en producción escondía tres bugs distintos, uno debajo del otro.** Después del
deploy nos pasó algo raro: un saludo simple al chat funcionaba perfecto, pero apenas alguien pedía
reservar de verdad contra Groq, explotaba con un `ClassCastException`. Tiramos del hilo y la causa
real estaba adentro de LangChain4j 1.0.0: `ChatModel.chat()` arma el request final combinando los
...
— es decir, en cualquier turno real del asistente, nunca en un saludo sin tool calling. Actualizamos
las cinco dependencias de LangChain4j a 1.2.0, donde el fix ya existe (issue #106, PR #107). Pero
arreglar eso destapó un segundo bug, esta vez nuestro: el wrapper de `ChatModel` diferido de
`BookingAssistantConfig` tenía el mismo patrón exacto de bug — delegaba en `doChat()` del modelo
real en vez de `chat()`, saltándose justo el merge de parámetros que acabábamos de arreglar (PR
#109). Y con esos dos ya resueltos, probar una reserva real *todavía* tiraba
`IllegalStateException: no hay usuario autenticado` — el problema del hilo de streaming que
contamos arriba, para el que en ese momento ni siquiera existía `SecurityContextToolExecutor` (PR
#111, con su propio test de regresión en #112). Los tres compartían la misma puerta de entrada
("reservar de verdad falla, saludar funciona") pero vivían en capas completamente distintas — una
versión de librería, un wrapper propio con el mismo antipatrón, y un límite de hilo sin cubrir.
Encontrarlos exigió reproducir el camino completo contra el proveedor real; los tests deterministas
con stub, que corren en CI, nunca los iban a atrapar porque no hay ningún hilo ajeno en un stub.

**La doble reserva bajo carga concurrente nos obligó a mirar más allá del código.** Validar
...
incluso si el código nunca la consultó. Ahí nos topamos con algo que no esperábamos: bajo inserts
...
primera corrida, ni en la segunda. Terminamos cubriendo dos SQLSTATE distintos (`23P01` y `40P01`),
no uno, y el detalle completo quedó en [ADR 0005](adr/0005-constraint-de-exclusion.md).

**El catálogo de modelos nos cambió el piso debajo de los pies, más de una vez.** El modelo de
Gemini que habíamos fijado al planificar (`gemini-2.5-flash`) dejó de estar disponible para cuentas
nuevas antes de que termináramos de implementar, redirigiendo primero a `gemini-3.6-flash` y
después a `gemini-3.7-flash` (que salió GA apenas once días antes) — lo verificamos en el momento
en vez de asumir el primer reemplazo que sugería el error 404
([ADR 0002](adr/0002-notebook-java-o-python.md)). Con Groq nos pasó lo mismo pero peor: los modelos
que trae `application.yml` como default (`llama-3.3-70b-versatile`, `llama-3.1-8b-instant`) quedaron
discontinuados en algún momento entre que escribimos el ADR del proveedor y la corrida de
evaluación en vivo, y recién lo notamos consultando el catálogo vigente contra la API real. Un
ecosistema de modelos gratuitos que se reorganiza en semanas, no en meses, nos dejó una lección
clara: fijar un nombre de modelo en configuración es una decisión con fecha de vencimiento
conocida, no un dato estático que se escribe una vez.

**El tier gratuito resultó bastante más chico de lo que esperábamos.** El spike de Gemini
([ADR 0009](adr/0009-limites-del-tier-gratuito-de-gemini.md)) nos mostró un límite de apenas 20
requests por día — muy por debajo de los 500-1500 que citaban blogs de terceros — y algo peor: los
intentos fallidos con `503` igual consumen cupo. Trece llamadas fallidas nos alcanzaron para quemar
14 de 20 solicitudes diarias en un solo spike, antes de que ninguna funcionara. Eso nos obligó a que
...
cupo — la mejor prueba de que el riesgo que habíamos anotado en el ADR era real, no una precaución
de más.

**Los errores del streaming no pueden cambiar el código de estado HTTP, y eso nos obligó a repensar
...
ya no hay forma de reescribir el status a un 503, como sí hace el endpoint síncrono. Resolvimos esto
...
**Un bug en nuestro propio arnés de evaluación nos hizo dudar de todo menos de él.** Las primeras
corridas de la suite de evaluación en vivo fallaban con cualquier API key que pusiéramos, y durante
un rato sospechamos de todo — la key, el proyecto de Google Cloud, la cuota — menos del código que
habíamos escrito para medir. La causa real era `RecordingChatModel` (el wrapper que graba qué tool
llamó el modelo, para comparar contra la esperada): delegaba en el método `doChat` de la interfaz
...
bug no era invisible — tirábamos una excepción, no un silencio — pero sí era engañoso, porque
parecía un problema de configuración y no del wrapper. Lo arreglamos antes de la corrida que quedó
documentada en `doc/eval/E07.3-resultados.md`.

**De-riesgamos el kernel de Jupyter contra un JDK recién salido del horno antes de comprometernos a
nada.** El enunciado da por sentado un notebook en Python; nosotros estábamos en Java 25, liberado
apenas antes de arrancar el challenge. Antes de apostar a un notebook en Java para la entrega final,
hicimos un spike temprano ([ADR 0002](adr/0002-notebook-java-o-python.md)) para verificar de punta
a punta que `rapaio-jupyter-kernel` arranca contra JDK 25, que su magia de dependencias resuelve un
artefacto real de LangChain4j desde Maven Central, y que una celda puede instanciar un `ChatModel`
real y llamarle a Gemini. Dejamos un plan B documentado (notebook en Python contra la API ya
desplegada) pero no hizo falta usarlo. Sacarnos esta duda en la primera semana, en vez de
descubrirla el último día, fue de las mejores decisiones de todo el proceso.

## Los huecos del enunciado, y qué decidimos en cada uno

El enunciado deja bastantes cosas sin especificar, y cada hueco necesitó un criterio propio, no una
convención genérica. Esto es lo que decidimos y por qué:

| Hueco | Qué decidimos | Dónde |
|---|---|---|
...
| El enunciado da por sentado un notebook en Python | Notebook en Java con `rapaio-jupyter-kernel`, verificado contra JDK 25 antes de comprometernos | [ADR 0002](adr/0002-notebook-java-o-python.md) |
| No hay caso de uso de "ver mis reservas" en el enunciado | Lo agregamos igual: `ListMyBookings`. Sin él, cancelar una reserva propia es impracticable — ni una persona ni el modelo tienen de dónde sacar el id a cancelar | [E04](epics/E04.md) |
...
| No se pide streaming ni panel de agenda | Los agregamos igual, como diferencial de UX (streaming vía SSE con `fetch`, no `EventSource`, porque este último no permite mandar el header `Authorization`) | [ADR 0010](adr/0010-streaming-del-chat.md) |
| No se pide observabilidad de las conversaciones del agente | Agregamos instrumentación opcional con Langfuse vía OpenTelemetry, apagada por defecto (`LANGFUSE_TRACING_ENABLED=false`), sin costo cuando está apagada | [ADR 0011](adr/0011-trazas-de-llm-y-tool-calls-con-langfuse.md) |

## Cómo sabemos que esto funciona de verdad

No queríamos quedarnos con "funcionó en la demo". El dominio tiene un test por regla — incluido el
...
gana — el mismo test que nos mostró el deadlock que no esperábamos. El agente tiene dos niveles de
...
en [`doc/eval/E07.3-resultados.md`](eval/E07.3-resultados.md). Así, si en algún momento cambiamos de
